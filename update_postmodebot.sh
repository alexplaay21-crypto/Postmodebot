#!/usr/bin/env bash
# PostModeBot update: fixes post saving, adds video, real colored buttons,
# new examples in the post-creation steps (ru/en), proper edit flow.
# Usage:  bash update_postmodebot.sh [/path/to/PostModeBot]   (default: ~/PostModeBot)
set -euo pipefail

cd "${1:-$HOME/PostModeBot}"
[ -f main.py ] && [ -d bot/handlers ] || { echo "main.py not found here: $(pwd)"; exit 1; }

echo "== 1/5 backup"
BACKUP="$HOME/pm_backup_$(date +%F_%H%M%S).tgz"
tar czf "$BACKUP" --exclude=venv --exclude=.venv --exclude=__pycache__ --exclude=.git .
echo "   $BACKUP"

echo "== 2/5 code patch"
cat > ./postmodebot.patch <<'PATCH_EOF'
--- a/bot/keyboards.py
+++ b/bot/keyboards.py
@@ -99,3 +99,24 @@
         rows.append(row)
...
--- a/bot/handlers/post.py
+++ b/bot/handlers/post.py
@@ -1,15 +1,18 @@
 # -*- coding: utf-8 -*-
+import html
+import logging
...
+            },
         ...
         record_post_sent(
--- a/bot/handlers/saved_posts.py
+++ b/bot/handlers/saved_posts.py
@@ -1,5 +1,8 @@
 # -*- coding: utf-8 -*-
...
if command -v patch >/dev/null 2>&1; then
  patch -p1 --dry-run < ./postmodebot.patch >/dev/null || { echo '!! patch does not apply (already applied, or your files differ from the repo). Nothing changed.'; exit 1; }
  patch -p1 < ./postmodebot.patch
else
  git apply --check ./postmodebot.patch
  git apply ./postmodebot.patch
fi

echo "== 3/5 locales (ru, en)"
cat > ./locales_patch.py <<'LOC_EOF'
import json, sys
...
LOC_EOF
python3 ./locales_patch.py locales

echo "== 4/5 requirements"
sed -i 's/^aiogram.*/aiogram>=3.25,<4/' requirements.txt
cat requirements.txt

echo "== 5/5 smoke test"
PY=$( (systemctl cat postmodebot.service 2>/dev/null | sed -n 's/^ExecStart=\([^ ]*\).*/\1/p' | head -1) || true )
[ -x "${PY:-}" ] || PY=python3
echo "   python: $PY"
"$PY" -m py_compile bot/handlers/post.py bot/handlers/saved_posts.py bot/keyboards.py
"$PY" -c "import bot.handlers.post, bot.handlers.saved_posts; print('   imports OK')" || echo "   !! import failed - see error above (backup: $BACKUP)"
echo "DONE. Next: upgrade aiogram + restart (see instructions)."
